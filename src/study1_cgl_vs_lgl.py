# -*- coding: utf-8 -*-
"""
Study 1: CGL vs LGL node comparison — pure bulk transport.

Uses the same transport setting as Study 0. Reference = finest FV WENO3
from Study 0 (must already exist on disk). Each (node_type, polyDeg, nelem)
config is run n_repeats times and the minimum wall time is kept.
"""
...
def study1_tests(n_jobs, small_test, output_path, cadet_path,
                 polydegs=None, node_types=None, n_repeats=3):
    """Run Study 1: CGL vs LGL pure transport convergence with min-time reruns.
    ...
    if not small_test:
        n_disc = {1: 10, 2: 10, 3: 8, 4: 7, 5: 7}
    else:
        n_disc = {p: 4 for p in range(1, 6)}

    base_model = setting_DG_transport.get_model()

    # FV reference = finest FV WENO3 from Study 0 (4 * 2^15 = 131072 cells)
    fv_ncol = 131072 if not small_test else (4 * 2**3)  # small: 32
    fv_ref_file = convergence.generate_1D_name(
        'radCol1D_FV_WENO3_transport_1comp', 0, fv_ncol)
    ...
    if not os.path.exists(fv_ref_path):
        ...
        print("Run Study 0 first to generate the FV WENO3 reference.")
        return
    ...
    try:
        for node_type in node_types_list:
            methods = []
            for p in poly_degs:
                methods.append((f'radCol1D_DG_{node_type}_transport_1comp_P{p}', p))
            addition = {
