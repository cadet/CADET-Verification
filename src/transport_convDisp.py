...
_reference_data_path_ = str(
    Path(__file__).resolve().parent.parent / 'data' / 'CADET-Core_reference' / 'transport'
)
...
def transport_tests(n_jobs, small_test,
    ...
    def refine_discretization(config_data, disc_idx, setting_name,
        ...
        if 'NonEq' in spatial_discretization:
            if spatial_discretization['NonEq']:
                ...
                x0 = 0.0 if axial_flow else config_data['input']['model']['unit_' + unit_id]['col_radius_inner']
                x1 = config_data['input']['model']['unit_' + unit_id]['col_length'] if axial_flow else config_data['input']['model']['unit_' + unit_id]['col_radius_outer']
    ...
    radial_ref = convergence.get_solution(
        radial_ref_file, unit='unit_001', which='outlet'
    )
