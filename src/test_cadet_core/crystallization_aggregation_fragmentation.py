...
output_path = str(project_repo.output_path / "test_cadet-core") + "/crystallization"
...
def calculate_normalized_error(ref, sim, x_ct, x_grid, n_x):
...
def crystallization_aggregation_EOC_test(cadet_path, small_test, output_path):
    ...
    for n_x in Nx_grid:
        ...
        normalized_l1.append(calculate_normalized_error(
            get_analytical_agg(n_x), sim, x_ct, x_grid, n_x))
...
def crystallization_fragmentation_EOC_test(cadet_path, small_test, output_path):
    ...
    for n_x in Nx_grid:
        ...
        sim = model.root.output.solution.unit_001.solution_outlet[-1, :]
        normalized_l1.append(calculate_normalized_error(
            get_analytical_frag(n_x, x_ct, cycle_time), sim, x_ct, x_grid, n_x))
...
def crystallization_aggregation_fragmentation_EOC_test(cadet_path, small_test, output_path):
    ...
    for n_x in Nx_grid:
        ...
        sim = model.root.output.solution.unit_001.solution_outlet[-1, :]
        normalized_l1.append(calculate_normalized_error(
            get_analytical_agg_frag(n_x, x_ct, cycle_time), sim, x_ct, x_grid, n_x))
