...
def generate_connections_matrix(rad_method, rad_cells,
    ...
    if add_inlet_per_port:
        ...
        nRadialZones = nRadPoints if add_inlet_per_port is True else add_inlet_per_port

        if not nRadPoints % nRadialZones == 0:  # todo: has to be elements here for DG
            raise Exception(
                f"Number of radial zones {nRadialZones} is not a multiple of nRadPoints {nRadPoints}")
...
def SamDiss_2DVerificationSetting(
    ...
    column.adsorption.lin_ka = 35.5
    column.adsorption.lin_kd = 1.0
